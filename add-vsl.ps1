...
# Automatiza:
# - detectar próximo vsl_00X
# - quebrar vídeo em HLS
...
# 1) Descobrir raiz do repo (onde o script está)
...
if (-not (Get-Command ffmpeg -ErrorAction SilentlyContinue)) {
    Write-Host "ERRO: ffmpeg não encontrado no PATH." -ForegroundColor Red
    Write-Host 'Instale com:  winget install "FFmpeg (Essentials Build)"'
    exit 1
}

# 3) Perguntar caminho do vídeo ou pasta
Write-Host ""
...
if (Test-Path $inputPath -PathType Container) {
    ...
    Write-Host "Caminho inválido: $inputPath" -ForegroundColor Red
    exit 1
}
...
# 6) Rodar ffmpeg para quebrar o vídeo
$segmentPattern = Join-Path $folderName 'segment_%03d.ts'
$outputIndex    = Join-Path $folderName 'index.m3u8'

Write-Host ""
Write-Host "Quebrando vídeo em HLS..." -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "ffmpeg retornou código $LASTEXITCODE. Algo deu errado." -ForegroundColor Red
    exit 1
}
...
if (-not (Test-Path $playerPath)) {
    Write-Host "player.html não encontrado na raiz. Pulei atualização." -ForegroundColor Yellow
} else {
    ...
    if ($content -match $pattern) {
        ...
        if ($items -notcontains $newItem) {
            ...
            Write-Host "Item $newItem já existe em allowed." -ForegroundColor Yellow
        }

        $newList = [string]::Join(', ', $items)
        $newContent = $content -replace $pattern, "const allowed = [$newList];"

        Set-Content $playerPath $newContent -Encoding UTF8
        Write-Host "player.html atualizado com $folderName." -ForegroundColor Green
    } else {
        Write-Host "Não encontrei linha 'const allowed = [...]' em player.html." -ForegroundColor Yellow
    }
}
...
if ($doGit -eq 's' -or $doGit -eq 'S') {
    try {
        ...
        Write-Host "Git push concluído." -ForegroundColor Green
    } catch {
        Write-Host "Falha ao executar git add/commit/push: $_" -ForegroundColor Red
    }
} else {
    Write-Host "Ok, sem git automático. Faça na mão depois." -ForegroundColor Yellow
}

Write-Host ""
Write-Host "Concluído. Nova VSL: $folderName" -ForegroundColor Cyan
